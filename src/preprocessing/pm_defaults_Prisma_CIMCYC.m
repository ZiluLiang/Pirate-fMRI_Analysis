...
%% Adaptation Info 
% This file is intended for use with the Siemens fieldmap sequence
% on the Prisma scanner at CIMCYC at University of Granada for the pirate
% fMRI project
%
% Adapted by Zilu Liang (2023.04)
...
pm_def.INPUT_DATA_FORMAT = 'PM';      % 'RI' = load two real and 
                                      ...
                                      % pairs.
pm_def.SHORT_ECHO_TIME = 4.92;        % Short echo time in ms 
pm_def.LONG_ECHO_TIME = 7.38;         % Long echo time in ms
pm_def.MASKBRAIN = 1;                 % Do brain masking (1 or 0,
                                      % 0 for EPI fieldmaps) Zilu: default was 1, can be changed to 0 if schur errs
...
pm_def.K_SPACE_TRAVERSAL_BLIP_DIR = -1;         % Meaning blip gradients are negative => PE direction (of prewinder is positive) 
                                                % Phase encoding direction:1 = P->A("j+" from json for the EPI image);-1 = A->P("j-" from json for the EPI image). +ve k-space = 1, -ve = -1.    
pm_def.TOTAL_EPI_READOUT_TIME = 0.519995*84;    % EPI RO time (0.5*64*(1 + Over Sampling Factor/100)) = 72 with OS = 12% 
                                                % Zilu: readout time = EffectiveEchoSpacing (in ms) * EchoTrainLength
                                                % Note that EffectiveEchoSpacingis a field in the json file after dcm2nii conversion,
                                                % otherwise look for EchoSpacing & Accelerate and calculate
                                                % EffectiveEchoSpacing = EchoSpacing/AccelerateFactor
