...
%------ adapted by Zilu Liang(2024.4,Oxford)------
clear;clc
...
raw_data_dir   = 'F:\Exp2_fmri\raw';

study_data_dir   = fullfile(wk_dir,'data','Exp2_pilotfmri');
study_script_dir = fullfile(wk_dir,'scripts','Exp2_pilotfmri');
...
filepattern    = struct('fieldmap',         'gre_field_mapping_2.5mm',...
                        't1',               'GIfMI_T1_MPRAGE',...
                        'pirate2AFC',       'ep2d_bold_RUN_[2-7]',...
                        'localizer2AFC',    'ep2d_bold_RUN_[1]',...
                        'symboloddball',    'ep2d_bold_RUN_[8-9]');
% Step control
img_convert  = true;
img_organize = true;
img_backup   = true;
...
renaming_pattern = struct('fieldmap',      'fmap-',...
                          't1',            'anat-T1w',...
                          'pirate2AFC',    'task-pirate2AFC_run-[1-6]',...
                          'localizer2AFC', 'task-localizer2AFC_run-1',...
                          'symboloddball', 'task-symboloddball_run-[1-2]');
...
function new_name = rename_converted_imgs(scan_type,old_name,new_id) %#ok<*DEFNU>
    renaming_pattern = struct('fieldmap',  'fmap-%s',...
                          't1',            'anat-T1w',...
                          'pirate2AFC',    'task-pirate2AFC_run-%s',...
                          'localizer2AFC', 'task-localizer2AFC_run-1',...
                          'symboloddball', 'task-symboloddball_run-%s');
    ...
    switch scan_type
        ...
        case 'pirate2AFC'
            ...
            new_name = sprintf(['sub-%s_',renaming_pattern.(scan_type)],new_id(4:end),parts{8}-1);  %% note that which part correspond to run no depends on the naming rule, double check before proceeding
        case 'symboloddball'
            parts = strsplit(old_name,'_');
            new_name = sprintf(['sub-%s_',renaming_pattern.(scan_type)],new_id(4:end),parts{8}-7);  %% note that which part correspond to run no depends on the naming rule, double check before proceeding
        otherwise
