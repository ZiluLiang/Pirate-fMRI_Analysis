...
trialdata = read.table(fn, header = TRUE, sep = ",")%>%
  filter(subid!="sub031"&subid!="sub027") # exclude this participant because fMRI data was incomplete
...
data_with_prob = loadRData(file.path(data_dir,"data_withprob_compo_lapse-0_01.RData"))%>%
  filter(subid!="sub031"&subid!="sub027")
...
learnerids = unique(filter(dividesub,expt_session==1&stage=="train"&LLR>=0)$subid)
generalizerids = unique(filter(dividesub,expt_session==1&stage=="test"&LLR>=0)$subid)
(nonlearnerids = unique(filter(dividesub,expt_session==1&stage=="train"&LLR<0)$subid))
(nongeneralizerids = unique(filter(dividesub,expt_session==1&stage=="test"&LLR<0)$subid))
...
substimLLR = aggregate_data(data_with_prob,
                              groupbyvars = c("cohort","subid","stage","expt_session","blocknumber_withinsess","stim_id"),
                              yvars = c("LLR_trial","LLRx_trial","LLRy_trial"),
                              stats = c("mu"),
                              additionalvars = c("expt_coordsys","expt_curricula","expt_session"))%>%
  change_cols_name(oldnames = c("mu_LLR_trial","mu_LLRx_trial","mu_LLRy_trial"),
...
substimLLR_mean = data_with_prob%>%
  ...
  mutate(nblock = max(blocknumber_withinsess))%>%
  aggregate_data(groupbyvars = c("cohort","subid","stage","expt_session","stim_id"),
                 yvars = c("LLR_trial","LLRx_trial","LLRy_trial"),
                 stats = c("mu"))%>%
  change_cols_name(oldnames = c("mu_LLR_trial","mu_LLRx_trial","mu_LLRy_trial"),
...
blockdata_sub = aggregate_data(data_with_prob,
  ...
  mutate(islearner = factor(islearner,levels = c(1,0),labels = c("learner","non-learner")),
         isgeneralizer = factor(isgeneralizer,levels = c(1,0),labels = c("generalizer","non-generalizer")))
...
```{r}
blockdata_sub = blockdata_sub%>%
  group_by(subid,expt_session,stage)%>%
  mutate(nblock = max(blocknumber_withinsess))%>%
  ungroup()

last_pretrain = filter(blockdata_sub,expt_session==1&blocknumber_withinsess==nblock)%>%
  dplyr::select(-c("nblock","expt_session","session_str","blocknumber_withinsess"))%>%
  pivot_longer(cols=c("distance","LLR","LLR_x","LLR_y","errorx","errory"),
               names_to = "performance_metric",
               values_to = "lastpretrain")

meanscanning  = filter(blockdata_sub,expt_session==3)%>%
  aggregate_data(groupbyvars = c("cohort","subid","stage","islearner","isgeneralizer","generalizer_label"),
                 yvars=c("distance","LLR","LLR_x","LLR_y","errorx","errory"),
                 stats = "mu")%>%
  change_cols_name(oldnames = paste0("mu_",c("distance","LLR","LLR_x","LLR_y","errorx","errory")),
                   newnames = c("distance","LLR","LLR_x","LLR_y","errorx","errory"))%>%
  pivot_longer(cols=c("distance","LLR","LLR_x","LLR_y","errorx","errory"),
               names_to = "performance_metric",
               values_to = "scanning")

pretrainscanner_behavsum_long = left_join(last_pretrain,meanscanning,by=c("cohort","subid","stage","islearner","isgeneralizer","generalizer_label","performance_metric"))
```
...
trainperf  = blockdata_sub%>%
  ...
  dplyr::select(-c("tmp_keeprow","blocknumber_withinsess","nblock","stage"))

testperf  = blockdata_sub%>%
  filter(stage=="test")%>%
  pivot_longer(cols=c("distance","LLR","LLR_x","LLR_y","errorx","errory"),
               names_to = "performance_metric",
               values_to = "test")%>%
  mutate(cycle = if_else(expt_session==1,blocknumber_withinsess,blocknumber_withinsess+6))%>%
  dplyr::select(-c("blocknumber_withinsess","nblock","stage"))

traintest_behavsum_long = left_join(trainperf,testperf,
                                    by=c("cohort","subid","expt_session","session_str",
                                         "islearner","isgeneralizer","generalizer_label",
...
p_trte_corr = traintest_behavsum_long %>%
  ...
  facet_nested_wrap(vars(cohort,session_str,cycle),nrow = 2,
            remove_labels="none",axes="all",
            )+
...
```{r}
last_pretrain = traintest_behavsum_long%>%
  filter(expt_session==1&cycle==6)%>%
  mutate(sess="last pretrain")%>%
  dplyr::select(-c("expt_session","session_str","cycle"))
average_scanning = traintest_behavsum_long%>%
  filter(expt_session==3)%>%
  aggregate_data(groupbyvars = c("cohort","subid","expt_session",
                                 "islearner","isgeneralizer","session_str",
                                 "generalizer_label","performance_metric"),
                 yvars = c("train","test"),
                 stats = "mu")%>%
  change_cols_name(oldnames = c("mu_train","mu_test"),newnames = c("train","test"))%>%
  mutate(sess="scanning")%>%
  dplyr::select(-c("expt_session","session_str"))
tetrlastpretrainscan = rbind(last_pretrain,average_scanning)

p_trte_corr_lprsc = tetrlastpretrainscan %>%
  filter(performance_metric=="LLR")%>%
  mutate(outlierlabel=if_else(test>0&train<0,subid,""))%>%
  ggplot(aes(x=train,y=test,color=isgeneralizer))+
  facet_nested_wrap(vars(cohort,sess),nrow = 2,
            remove_labels="none",axes="all",
            )+
  geom_point(alpha=0.7)+
  geom_text(aes(label=outlierlabel))+
  ...
  geom_hline(yintercept=0,linetype="dashed",color="grey")+
  scale_color_manual(values=twolevel_colors)+
  labs(x="training stimuli LLR",y="test stimuli LLR", title="Correlation between LLR of Training and Test stimuli")+
  theme(legend.position = "top",legend.direction = "horizontal")
ggsave(file.path(result_dir,paste0('LLR_corr_train2test_lprsc','.png')),plot = p_trte_corr_lprsc,device = 'png',width = 8,height = 9)
```
