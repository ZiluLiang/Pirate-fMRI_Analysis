...
############################### loop over stimuli category and roi and run for 10 times to generate average decoding performance
n_repetition = 10
n_epoch = 100
ds_name="oddeven"
...
with Parallel(n_jobs=10) as parallel:
    for dcode_name,dcode_cfg in decode_type.items():
        ...
        for flist_name, flist in filters.items():
            decoder_output[flist_name] = {}
            for roi in roi_dict.keys():
                print(f"{flist_name}-{roi}")
                decoder_output[flist_name][roi] = []
                for subid,bdf,op,f in zip(subid_list,behav_dfs[ds_name],neuralrdm_rops[ds_name][roi],flist):
                    rep_output_list = parallel(delayed(MLP_decoding_cv)(
                                                        op[0][f,:],
                                                        bdf[f].reset_index(),
                                                        readout_colnames,splitgroup_colnames,n_hidden=20,n_epoch=n_epoch) for jrep in range(n_repetition))
                    losses_ = {"fit":  np.mean([op[0]["fit"].mean(axis=0) for op in rep_output_list],axis=0),
                               "eval": np.mean([op[0]["eval"].mean(axis=0) for op in rep_output_list],axis=0)}
                    
                    scores_ = {"fit":  np.mean([op[1]["fit"] for op in rep_output_list]),
                               "eval": np.mean([op[1]["eval"] for op in rep_output_list])}
                    
                    max_evalscore_run = np.argmax([op[1]["eval"] for op in rep_output_list])
                    max_behavior = rep_output_list[max_evalscore_run][2]

                    decoder_output[flist_name][roi].append([losses_, scores_, max_behavior])

        output_dir = os.path.join(ROIRSA_output_path,"decoding","navigation")
        checkdir(output_dir)
        fn = os.path.join(output_dir,f"averageof{n_repetition}runs_{dcode_name}_X.pkl")
        dump(decoder_output,filename=fn)