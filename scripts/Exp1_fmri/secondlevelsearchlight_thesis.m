...
%% Extract estimates from parietal
mask_fn = fullfile(directory.fmri_data,"ROIRSA/AALandHCPMMP1andFUNCcluster","testgtlocParietalSup_bilateral.nii");
lzer_gtloc_dir = fullfile(par_dirs{2},"regression","compete_featurecartesian_combinexy");
training_gtloc_dir = fullfile(par_dirs{1},"regression","compete_featurecartesian_combinexy_trainingstim");
test_gtloc_dir = fullfile(par_dirs{1},"regression","compete_featurecartesian_combinexy_teststim");

rsa_dirs = struct("localizer",lzer_gtloc_dir,"trainingstim",training_gtloc_dir,"teststim",test_gtloc_dir);

metric_names = structfun(@(x) fieldnames(loadjson(fullfile(x,'first',participants.validids{1},'searchlight.json')).estimator.modelRDMs),rsa_dirs,'uni',0);
img_fns = structfun(@(names) sprintf("beta_%04d.nii%", find(cellfun(@(x) contains(x,"gtlocEuclidean"),names))-1), metric_names,'uni', 0 ); % because python is zero indexing, imagine start from 0

stimgroups = fieldnames(rsa_dirs);
gtloc_betas = struct();
for j = 1:numel(stimgroups)
    rsadir = rsa_dirs.(stimgroups{j});
    
    gtloc_betas.(stimgroups{j}) = spm_summarise(cellstr(fullfile(rsadir, "first", participants.validids, img_fns.(stimgroups{j}) )), ...
              char(mask_fn), @(x) mean(x,"omitnan"));
end

gtloc_betaweights = struct2table(gtloc_betas);
gtloc_betaweights.subid = participants.validids;

writetable(gtloc_betaweights,fullfile(directory.fmri_data,"ROIRSA/AALandHCPMMP1andFUNCcluster","wbsearchlight_meangtlocweight_ParietalSup.csv"))

%% Extract estimates from vmPFC
mask_fn = fullfile(directory.fmri_data,"ROIRSA/AALandHCPMMP1andFUNCcluster","vmPFC_bilateral.nii");
lzer_gtloc_dir = fullfile(par_dirs{2},"regression","compete_featurecartesian_combinexy");
training_gtloc_dir = fullfile(par_dirs{1},"regression","compete_featurecartesian_combinexy_trainingstim");
test_gtloc_dir = fullfile(par_dirs{1},"regression","compete_featurecartesian_combinexy_teststim");
...
writetable(gtloc_betaweights,fullfile(directory.fmri_data,"ROIRSA/AALandHCPMMP1andFUNCcluster","wbsearchlight_meangtlocweight_vmPFC.csv"))