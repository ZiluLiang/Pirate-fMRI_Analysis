...
masks = cellstr(spm_select('FPList','E:\pirate_fmri\Analysis\data\Exp1_fmri\fmri\ROIRSA\AALandHCPMMP1','.*_bilateral.nii'));
...
for jdir = 1%:numel(lsa_dir)     
    if flag_runGLM
        for j = 1:numel(LSAglm_names) %#ok<*UNRCH>
            ...
            checkdir(glm_dir)
            subidlist       = {'sub118'};%participants.validids;
...
for j = 1%:numel(LSAglm_names)
    ...
    %     extract_firstlvl_spmStat(glm_name,fullfile(directory.fmri_data,lsa_dir{1},sprintf('%sC1',glm_name)), ...
    %                              repmat(masks(1),numel({glm_configure(glm_name).contrasts.name}),1));
...
for jdir = 1:numel(lsa_dir)
    ...
    for isub  = 1:nsub
        ...
        if all(~isempty(reg_img))%~any(cellfun(@isempty,[contrast_img1;contrast_imgo;contrast_imge;reshape(reg_img,[],1)]))
...
%         fprintf('Concatenating 4D activity pattern images for %s\n',subidlist{isub})
% 
%         firstlvl_dir = structfun(@(x) fullfile(x,'first',subidlist{isub}),concat_glm_dir,'UniformOutput',false);
