...
analysis = {'regression','correlation','composition','cosinesimilarity','decoding_AxisLocContinuous','decoding_AxisLocDiscrete'};   
        ...
        for j_rsadir = 2%1:numel(rsa_dirs)
            ...
            switch analysis{j_analysis}
                ...
                case "regression"
                    ...
                    trans_formula = '';
                case "composition"
                    metric_names = fieldnames(loadjson(fullfile(rsa_dir,'first',participants.validids{1},'searchlight.json')).estimator.estimator.modelRDMs);
                    metric_idxs = 1:numel(metric_names);
                    img_regexp ="rho_%04d.nii";
                    get_new_imgname = @(x) strcat('transformed_',x);                
                    trans_formula = '';
