...
The decoding is done using a logistic regression classifier with a stratified k-fold crossvalidatiion.
During decoder fitting, hyperparameter C is optimized using a gridsearchCV approach
...
# For saving
save_fn = "noncenter_trainingstim_LRdecoding_acc_skf"
save_dir = os.path.join(ROIRSAdir,"trainingstim_decoding_results")
checkdir(save_dir)
flag_saveGSres = True
flag_saveCM = True
n_jobs = 10
        ...
        for subdata,subid in zip(roi_data[roi],subid_list):
            ...
            for tarvar in ["stim_x","stim_y","stim_id"]:
                ...
                #for heldout_sessions in np.unique(session_labels):
                    ...
                    GSres.append(pd.DataFrame(clf.cv_results_).assign(roi=roi,subid=subid,target=tarvar,straifiedfold=i))#heldoutrun=heldout_sessions))
