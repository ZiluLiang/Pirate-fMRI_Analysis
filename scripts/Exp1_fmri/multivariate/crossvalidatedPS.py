...
def PTA_RSA_CV(subpsX, subrsaX, subdf, heldoutgroups):
...
for pgroi in rois:
    ...
    for jsub, (subpsX,subdf,subid) in enumerate(zip(sub_patterns[pgroi],sub_stimdfs[pgroi],subid_list)):
        ...
        subpsX_oddeven = average_odd_even_session(subpsXdemean,subdf.stim_session)
        ...
        subrsaX_oddeven = average_odd_even_session(subrsaXdemean,subdf.stim_session)
        subcvres, subfitPS, subevalPS = PTA_RSA_CV(subpsX_oddeven, subrsaX_oddeven, subdf[subdf.stim_session<2].copy(), heldoutgroups=1) #[subdf.stim_session<2].copy()
...
dump({"RSA":ROIRSA_PTA_cvres,"PS":ROIPS_PTA_cvres},os.path.join(ROIRSAdir,"ROI_PTARSA_cvoddeven_rowzscore.pkl"))        
