...
import scipy
import pandas as pd
from sklearn.model_selection import LeaveOneGroupOut, LeavePGroupsOut, RepeatedStratifiedKFold
...
def PTA_RSA_CV(subpsX, subrsaX, subdf):
    res_list = []
    PS_fits, PS_evals  = [], []
    axlocs = subdf.training_axlocTL.values
    eval_pairs = [(-2,-1),(-2,1),(-2,2)]
    for eval_pair in eval_pairs:
        fit_pair = [x for x in subdf.training_axlocTL.unique() if x not in eval_pair]
        fit_index = np.array([x in fit_pair for x in axlocs])
        eval_index = np.array([x in eval_pair for x in axlocs])
        fitPSX  = subpsX[fit_index,:]
        
        #subfitPS = np.mean([cal_PS(sX,subdf[subdf.stim_session==0].copy()) for sX in fitX_splits])
        subfitPS = cal_PS(fitPSX,subdf[fit_index].copy().reset_index())
        ...
        stim_dict = subdf[eval_index].copy().reset_index(drop=True).to_dict('list')
        ...
        for analysis_dict in analyses:
            ...
            R2    = reg_estimator.R2,
            heldoutpair = f"{eval_pair[0]}_{eval_pair[1]}"
            )
            res_list.append(res_df)
        
        # do PS for heldout as well to see if they aligned
        subevalPS = cal_PS(evalRSA_X,subdf[eval_index].copy())
...
for pgroi in rois:
    ...
    for jsub, (subpsX,subdf,subid) in enumerate(zip(sub_patterns[pgroi],sub_stimdfs[pgroi],subid_list)):
        ...
        #for roi in rois:
        roi = pgroi
        subRSAX = deepcopy(subpsX_ave)
        subcvres, subfitPS, subevalPS = PTA_RSA_CV(subpsX_ave, subRSAX, subdf[subdf.stim_session<1].copy())
...
dump({"RSA":ROIRSA_PTA_cvres,"PS":ROIPS_PTA_cvres},os.path.join(ROIRSAdir,"ROI_PTARSA_locpairheldout.pkl"))        
