...
rois =  [f"{x}_bilateral" for x in roi_data.keys()] + ["allgtlocPrecentral_left"]

resultdir = os.path.join(ROIRSAdir,"axisPS")


############ Load the null
sub_cs_perms = load(os.path.join(ROIRSAdir,"axisPS","aveactivitypatPS_NCtrainingloc.pkl"))["permutation"]
...
def compute_RSA(subPScat,subX,subdf):
    ...
    analyses = [
            ...
            # {
            # "name": "train stim betweenxy PTA high",
            # "reg_names": [f"betweenxy_gtlocEuclidean"],
...
def split_fiteval_data(subX,subdf,evalpair):#
    axlocs= subdf.training_axlocTL
    assert np.array_equal(np.unique(axlocs),np.array([-2,-1,1,2]))
    fitpair = [x for x in np.unique(axlocs) if x not in evalpair]
    fit_index = np.array([x in fitpair for x in axlocs])
    eval_index = np.array([x in evalpair for x in axlocs])

    return subX[fit_index], subX[fit_index], subdf[fit_index].copy().reset_index(drop=True), subdf[eval_index].copy().reset_index(drop=True)


def calculate_group_mean(subXs,subdfs,subid_list,PSnull,randseed,returnsubdfs=False, verbose=False):
    if verbose:
    ...
    for jsub, (subX,subdf,subid,eval_pair) in enumerate(zip(subXs,subdfs,subid_list,subeval_pairs)):
        subX_ave = np.mean(split_data(subX,subdf.stim_session),axis=0)
        subdf_singlesess = subdf[subdf.stim_session==0].copy().reset_index(drop=True)
        
        subX_fit, subX_eval, subdf_fit, subdf_eval = split_fiteval_data(subX_ave,subdf_singlesess,eval_pair)
...
n_perm = 5000
save_fn = "ROI_PTARSA_CV"
roi_res = {}
with Parallel(n_jobs=10) as parallel:
    for pgroi in rois:
        print(f"Calculating for {pgroi}",end="\n")
        rsaroi = pgroi
        roi_res[pgroi] = parallel(delayed(calculate_group_mean)(sub_patterns[pgroi],sub_stimdfs[pgroi],subid_list,sub_cs_perms[pgroi],perm,True,True) for perm in range(n_perm))
...
# ########## plot the results ##########
# roi_res = load(os.path.join(resultdir,"ROI_PTARSA_CV.pkl"))
# groupave_dfs = {}
# groupave_dfs["HPC_bilateral"] = []
# for _, allsubdf in roi_res["HPC_bilateral"]:
#     allsubdf["subgroup"] = ["Generalizer" if x in generalizers else "nonGeneralizer" for x in allsubdf.subid]
#     groupavedf = allsubdf.groupby(["subgroup","fitPScat","analysis","modelrdm"])["coefficient"].mean().reset_index()
#     groupave_dfs["HPC_bilateral"].append(groupavedf)

# groupO_pta = np.concatenate([df[(df.analysis=="train stim Compete PTAhigh Cartesian")&(df.modelrdm=="PTA_locNomial")&(df.fitPScat==0)&(df.subgroup=="Generalizer")].copy()["coefficient"].values for df in groupave_dfs["HPC_bilateral"]])
# groupO_euc = np.concatenate([df[(df.analysis=="train stim Compete PTAhigh Cartesian")&(df.modelrdm=="gtlocEuclidean")&(df.fitPScat==0)&(df.subgroup=="Generalizer")].copy()["coefficient"].values for df in groupave_dfs["HPC_bilateral"]])
# groupPS_pta = np.concatenate([df[(df.analysis=="train stim Compete PTAhigh Cartesian")&(df.modelrdm=="PTA_locNomial")&(df.fitPScat!=0)&(df.subgroup=="Generalizer")].copy()["coefficient"].values for df in groupave_dfs["HPC_bilateral"]])
# groupPS_euc = np.concatenate([df[(df.analysis=="train stim Compete PTAhigh Cartesian")&(df.modelrdm=="gtlocEuclidean")&(df.fitPScat!=0)&(df.subgroup=="Generalizer")].copy()["coefficient"].values for df in groupave_dfs["HPC_bilateral"]])
# rsacv_res = pd.concat([
