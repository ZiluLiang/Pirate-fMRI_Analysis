    ...
    def fit(self):
        ...
        for regname in self.run_reg:
            ...
            for cvsplit,[[fitf_tar,fitf_X],[evalf_tar,evalf_X]] in splitter.items():
                ...
                # do it based on RDM so that it is rotation invariant
                assert compute_rdm(fit_tar.T,"correlation").shape[0] == np.sum(fitf_tar)
                fittar_rdm   = lower_tri(compute_rdm(fit_tar.T,"correlation"))[0]
                fitpred_rdm  = lower_tri(compute_rdm(fit_pred.T,"correlation"))[0]
                evaltar_rdm  = lower_tri(compute_rdm(eval_tar.T,"correlation"))[0]
                evalpred_rdm = lower_tri(compute_rdm(eval_pred.T,"correlation"))[0]

                reg_res[f"fit_r2_{cvsplit}"]  = r2_score(fittar_rdm,fitpred_rdm)
                reg_res[f"eval_r2_{cvsplit}"] = r2_score(evaltar_rdm,evalpred_rdm)                
                reg_res[f"fit_corr_{cvsplit}"] = scipy.stats.spearmanr(fittar_rdm,fitpred_rdm).statistic
                reg_res[f"eval_corr_{cvsplit}"] = scipy.stats.spearmanr(evaltar_rdm,evalpred_rdm).statistic
...
restuldir = os.path.join(ROIRSAdir,"retrievalpatternCV_rdmbased")
checkdir(restuldir)
...
if organize_and_get_data:
    ...
    # Get the data for shuffled lzer
    import time
    with Parallel(n_jobs=20) as parallel:
        print("Getting original and shuffled data")
        for roi in rois:
            ...
            for Xs,dfs,subid in zip(allsubXs[roi],allsubdfs[roi],subid_list):
                ...
                starttime= time.time()
                lzer_shuffled_Xs = [np.vstack([np.random.default_rng(p+j*100000).permutation(x) for j,x in enumerate(lzer_X)]) for p in range(n_perm)]
                print(f"Shuffling {n_perm} localizer data in {roi} - {subid} took {time.time()-starttime:.2f}s",end="\r",flush=True)
                
                shufflelzerXs.append(lzer_shuffled_Xs)
                

            checkdir(os.path.join(restuldir,roi))
            print(f"\nSaving rotated data for {roi}")
            dump({"original_Xs":subXs,
                  "original_dfs":subdfs,
                  "shuffled_lzerXs":shufflelzerXs
                },
                os.path.join(restuldir,roi,f"shuffled_datasets.pkl")
                )


if run_sc:
    checkdir(os.path.join(restuldir,"sanitycheck"))
    from scipy.stats import pearsonr
    ...
    for roi in rois:
        shuffled_datasets = load(os.path.join(restuldir,roi,"shuffled_datasets.pkl"))
        subXs, subdfs = shuffled_datasets["original_Xs"], shuffled_datasets["original_dfs"]
        shufflelzerXs = shuffled_datasets["shuffled_lzerXs"]
        ...
        r2OE["test"][roi] = [r2_score(Xs[1][dfs[1].stim_session.values==0],
                                      Xs[1][dfs[1].stim_session.values==1]) for Xs,dfs in zip(subXs, subdfs)]
        rdm_corrOE["train"][roi] = [compare_rdm(Xs[0][dfs[0].stim_session.values==0],
                                                Xs[0][dfs[0].stim_session.values==1]) for Xs,dfs in zip(subXs, subdfs)]
        r2OE["train"][roi] = [r2_score(Xs[0][dfs[0].stim_session.values==0],
                                      Xs[0][dfs[0].stim_session.values==1]) for Xs,dfs in zip(subXs, subdfs)]

        # run sanity checks to see if shuffle  changed the representation structure as it should
        rdm_sc["shuffled"][roi] = [[compare_rdm(sX, Xs[2]) for sX in sXs] for Xs,sXs in zip(subXs,shufflelzerXs)]
    ...
    for roi in rois:
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(restuldir,"sanitycheck",f"{roi}_OErdmcorr.png"))


    for roi in rois:
        
        scshuffleddata = np.mean(
            [x for x,subid in zip(rdm_sc["shuffled"][roi],subid_list) if subid in generalizers],
            axis=0
        )
        xlims = (np.min(scshuffleddata)*0.999,
                 np.max(scshuffleddata)
                 )
        ticks = list(np.round(np.linspace(xlims[0],xlims[1],num=5,endpoint=True),2)) #+ [0]
        fig = plt.figure(figsize=(10,5))
        plt.subplot(1,1,1)
        plt.hist(scshuffleddata,color=participantgrouphex["Generalizer"],alpha=0.4)
        plt.xlim(xlims)
        plt.xticks(ticks)
        plt.axvline(np.mean(scshuffleddata),color=participantgrouphex["Generalizer"],linestyle="--",label="mean")
        plt.xlabel("correlation", fontsize=12,fontweight="bold")
        plt.ylabel("count", fontsize=12,fontweight="bold")
        
        
        fig.suptitle(f"Before and After Shuffling Localizer Task RDM Correlation\n{roi_shortnames[roi]}", fontweight="bold", fontsize=16) 
        fig.tight_layout()
        fig.savefig(os.path.join(restuldir,"sanitycheck",f"{roi}_shufflerdmcorr.png"))
...
if run_ana:
    ...
    def runrsaforshuffledlzer(p,Xs,dfs,subid_list,run_regs,outputdir):
        print(f"\nRunning Shuffle {p}",end="\r",flush=True)
        _, resultdf   = RPCV_run_group(Xs,dfs,subid_list,run_regs) 
        curr_perm_res = resultdf.assign(roi=roi,dataset="shuffled",perm=p)
    ...
    with Parallel(n_jobs=20) as parallel:
        for roi in rois:
            ...
            original_reg_coefs, original_results = RPCV_run_group(subXs,subdfs,subid_list,run_regs)    
            original_results = original_results.assign(roi=roi,dataset="orginal")
            original_results.to_csv(os.path.join(restuldir,roi,"original_results.csv"),index=False)
            dump(original_reg_coefs,os.path.join(restuldir,roi,"original_reg_coefs.pkl"))
            ...
            shuffle_Xs = [ [ [Xs[0],Xs[1], sXs[p]] for Xs,sXs in zip(subXs,shufflelzerXs)] for p in range(n_perm)]
            parallel(delayed(runrsaforshuffledlzer)(p,Xs,subdfs,subid_list,run_regs,shulffledoutputdir) for p,Xs in zip(range(n_perm),shuffle_Xs))
...
for roi in rois:
    print(roi,end="\n")
    original_resultsdf = pd.read_csv(os.path.join(restuldir,roi,"original_results.csv"))
    shuffled_dfs = []
    for p in range(n_perm):
        shuffled_dfs.append(pd.read_csv(os.path.join(restuldir,roi,"shuffledlzer",f"perm{p}_results.csv")))
    shuffledlzer_results_df = pd.concat(shuffled_dfs).reset_index(drop=True)
    shuffledlzer_results_df_sum = shuffledlzer_results_df.groupby(["dataset","roi","subid","metric"])["value"].mean().reset_index()
   
   
    resoverview_dfs.append(
        pd.concat(
            [original_resultsdf.assign(perm=np.nan),
             shuffledlzer_results_df
            ]
    ...
    checkmetrics = ["fit_corr","fit_r2","eval_corr","eval_r2","compoweight","meanweightdiff","reg_percept","reg_distmod"]
    ...
    gs = sns.catplot(data = lzerpred_df,
                    x=xvar,y=yvar,hue=huevar,palette = palettes, sharex=False,sharey=False,
                    col="metricname",col_wrap=2, 
                    kind="point",errorbar="se",capsize=0.1,markersize=0,linestyle="none",dodge=True
                    )
    gs.map_dataframe(sns.swarmplot,x=xvar,y=yvar,hue=huevar,palette = palettes,dodge=True)
    gs.fig.suptitle(f"{roi}: Localizer to Test Retrieval Performance",y=1.05)
    for gname,gdf in lzerpred_df.groupby(["metricname","dataset","subgroup"]):
        print(gname)
    ...
    # plot train 2 test
    navipred_df = retrievalcvdf[(retrievalcvdf.analysis=="train2test")&(retrievalcvdf.metricname.isin(checkmetrics))].copy()
    navipred_df.metricname = pd.Categorical(navipred_df.metricname, categories=checkmetrics,ordered=True)
    xvar, yvar, huevar = "subgroup", "value", "dataset"
    palettes = dict(zip(navipred_df[huevar].unique(),
                        sns.color_palette(None,navipred_df[huevar].nunique())
                        ))
    gs = sns.catplot(data = navipred_df,
                    x=xvar,y=yvar,hue=huevar,palette = palettes,
                    col="metricname",col_wrap=2, sharex=False,sharey=False,
                    kind="point",errorbar="se",capsize=0.1,markersize=0,linestyle="none",dodge=True
                    ).set_titles("{col_name}")
    gs.map_dataframe(sns.swarmplot,x=xvar,y=yvar,hue=huevar,palette = palettes,dodge=True)
    for gname,gdf in navipred_df.groupby(["metricname","dataset","subgroup"]):
        print(gname)
        print(scipy.stats.ttest_1samp(gdf.value.values,0))
    gs.fig.suptitle(f"{roi}: Train to Test Retrieval Performance",y=1.05)
    gs.savefig(os.path.join(restuldir,"resoverview",f"{roi}_train2test.png"))


original_reg_coefs = dict(zip(rois, [{} for _ in rois]))
for roi in rois:
    original_reg_coefs[roi] = load(os.path.join(restuldir,roi,"original_reg_coefs.pkl"))


dump(
    {
       "results": pd.concat(resoverview_dfs).reset_index(drop=True),
       "retrieval_coefs": original_reg_coefs
    },
