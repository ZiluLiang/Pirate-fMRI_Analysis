...
# first we get data for each stimulisubset we want to analyse
sort_by_ccols = {
...
# first we obtain the data and rdm for each group
print("Obtaining data and RDMs\n",end="\n")
sub_rdms = {}
stimdfs  = {}
for s, ssname in enumerate(sort_by_ccols.keys()):
    ...
    for ir,roi in enumerate(rois):
        ...
        for cname,clist in cohort_names_lists.items():            
            Gdata  = [sort_activitypattern_with_stimdf(*get_activity_pattern_for_stimsubset(subdata,ssname), sortby=sort_by_ccols[ssname]) for subid,subdata in zip(subid_list,roi_data[roi]) if np.logical_and(subid in generalizers,subid in clist)]
            nGdata = [sort_activitypattern_with_stimdf(*get_activity_pattern_for_stimsubset(subdata,ssname), sortby=sort_by_ccols[ssname]) for subid,subdata in zip(subid_list,roi_data[roi]) if np.logical_and(subid in nongeneralizers,subid in clist)]

            sub_rdms[ssname][roi][cname] = {}            
            sub_rdms[ssname][roi][cname]["G"]  = [compute_rdm(subd[0],"correlation") for subd in Gdata]
            sub_rdms[ssname][roi][cname]["nG"] = [compute_rdm(subd[0],"correlation") for subd in nGdata]
...
for s, ssname in enumerate(sort_by_ccols.keys()):
    ...
    for ir,roi in enumerate(rois):
        ...
        for cname,clist in cohort_names_lists.items():
            average_rdms[ssname][roi][cname] = {}
            mds_results["2D"][ssname][roi][cname] = {} 
            mds_results["3D"][ssname][roi][cname] = {}            
            
            average_rdms[ssname][roi][cname]["G"]  = np.mean([minmax_scale(m,newmin=0,newmax=1) for m in sub_rdms[ssname][roi][cname]["G"]],axis=0)
            average_rdms[ssname][roi][cname]["nG"] = np.mean([minmax_scale(m,newmin=0,newmax=1) for m in sub_rdms[ssname][roi][cname]["nG"]],axis=0)

            for sg in ["G","nG"]:
                for ncompo in [2,3]:
                    stimdf = stimdfs[ssname].copy()
                    rescaled_ave_rdm = average_rdms[ssname][roi][cname][sg]
                    mdsres = np.array(matlab_eng.cmdscale(rescaled_ave_rdm,ncompo))
                    stimdf[[f"MDS{i+1}" for i in range(ncompo)]] = mdsres
                    mds_results[f"{ncompo}D"][ssname][roi][cname][sg] = stimdf

matlab_eng.quit()
dump({"aveRDM":average_rdms,"MDSres":mds_results},os.path.join(ROIRSAdir,"mds_results_rescaled.pkl"))