...
for j = 1:numel(LSAglm_names)
    glm_name = LSAglm_names{j};    
    [~,meanResMS1.(glm_name),~] = extract_firstlvl_spmStat(glm_name,fullfile(directory.fmri_data,lsa_dir{1},glm_name),masks);
    [~,meanResMS2.(glm_name),~] = extract_firstlvl_spmStat(glm_name,fullfile(directory.fmri_data,lsa_dir{2},glm_name),masks);
end

%% run LSA beta series extrator GLMs (concatentaed)
