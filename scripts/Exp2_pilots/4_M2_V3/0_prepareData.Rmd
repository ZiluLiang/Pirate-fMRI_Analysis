...
ggplot(data=attention_check_sum,aes(x=x.label,y=accuracy,fill=session_str))+
  ...
  labs(fill="session",title="attention check accuracy",x="subid")

filter(attention_check_sum,accuracy<0.5)$subid #==min(attention_check_sum$accuracy)
```
```{r}
remove_sub = filter(attention_check_sum,accuracy<0.5)$subid 
...
#
classifyl_session = 1# use final block of refresher
classifyl_stype = "training"
learnerids = unique(filter(dividesub,expt_session==classifyl_session&stim_group==classifyl_stype&sum_LLR>=0)$subid)
(nonlearnerids = unique(filter(dividesub,expt_session==classifyl_session&stim_group==classifyl_stype&sum_LLR<0)$subid))

classifyg_session = 2# use final block of test
classifyg_stype = "validation"
generalizerids = unique(filter(dividesub,expt_session==classifyg_session&stim_group==classifyg_stype&sum_LLR>=0)$subid)
(nongeneralizerids = unique(filter(dividesub,expt_session==classifyg_session&stim_group==classifyg_stype&sum_LLR<0)$subid))
