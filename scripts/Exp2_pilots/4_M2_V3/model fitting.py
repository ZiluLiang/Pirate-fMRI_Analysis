...
for id in valid_exptid:
    ...
    n_refresh_permap = 4
    n_2AFC = 6
    final_refresher_idx = n_pretrain_permap*n_map+n_refresh_permap-1
    test_block_idx = n_pretrain_permap*n_map+n_refresh_permap+n_2AFC
    ...
    sub_trainingresp=subdf[(subdf.expt_session==1)&(subdf.expt_block==final_refresher_idx)].copy().reset_index(drop=True)
...
if check_fit:
    ...
    model_paramnames = fitted_data["model_paramnames"]
    model_xystrategies = fitted_data["model_xystrategies"]
    unique_fitparam_names = np.unique(sum(list(model_paramnames.values()),[]))
    unique_fixparam_names = ["betax", "betay","biasx","biasy","lapse_rate_2D"]

    all_param_names = np.array(list(unique_fitparam_names) + unique_fixparam_names)
    ...
    for jid,(id,fit_output) in enumerate(zip(valid_exptid,fit_param)):
        ...
        for pname in unique_fitparam_names:
