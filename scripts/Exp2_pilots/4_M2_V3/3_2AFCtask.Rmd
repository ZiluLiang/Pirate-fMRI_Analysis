...
fc_df = filter(fc_df_all,missed==0&islearner=="learner")
...
```{r,fig.width=5,fig.height=5}
blockfcdata_sub = aggregate_data(filter(fc_df,resp_correct=TRUE),
                           yvars = c("resp_rt","distractor_dist"),
                           groupbyvars = c("subid","expt_maptype","stim_group","session_str","blocknumber_withinsess"),
                           stats = c("mu"),
                           additionalvars = c("blocknumber_withinsess","session_str","islearner","isgeneralizer","iscompositional"))%>%
  change_cols_name(oldnames = c("mu_resp_rt","mu_distractor_dist"),
                   newnames = c("blockrt","blockdist"))

blockfcdata_sum = aggregate_data(blockfcdata_sub,
                           yvars = c("blockrt","blockdist"),
                           groupbyvars = c("session_str","expt_maptype","iscompositional","stim_group","blocknumber_withinsess"),
                           additionalvars = c("session_str","blocknumber_withinsess"),
                           stats = c("mu","se"))
(p=
   ggplot(data = blockfcdata_sum,
       aes(x=blocknumber_withinsess,y=mu_blockrt,color=stim_group))+
  facet_nested(rows=vars(iscompositional),
               cols=vars(expt_maptype),
               scales = "free_x",space = "free_x")+
  ...
  geom_line(position = position_dodge(width=1),linewidth=0.8)+
  geom_line(data=blockfcdata_sub,
            aes(y=blockrt,group=interaction(stim_group, subid)),
            position=position_dodge(width=0.5),
            alpha = 0.5,linewidth=0.4)+
  geom_point(data=blockfcdata_sub,
             aes(y=blockrt,group=interaction(stim_group, subid)),
...
```{r}
model_acc  = glmer(resp_correct~blocknumber_withinsess+iscompositional*iscenter+expt_maptype+distractor_tsrespdist + (iscenter|subid)+(distractor_tsrespdist|subid) + (1|stim_id),
              data=fc_df,family = binomial) 
model_rt   = lmer(resp_rt~blocknumber_withinsess+iscompositional*iscenter+expt_maptype+distractor_tsrespdist + (iscenter|subid)+(distractor_tsrespdist|subid) + (1|stim_id),
...
map_switch_sub = fc_df_switch%>%
  aggregate_data(groupbyvars = c("subid","iscenter","stimtype_switch"),
                 yvars = c("resp_correct","resp_rt"),
...
map_switch_sum = map_switch_sub%>%
  aggregate_data(groupbyvars = c("iscompositional","iscenter","stimtype_switch"),
                 yvars=c("accuracy","rt"),
                 stats=c("mu","se"))

ggplot(data=map_switch_sum,aes(x=iscenter,color=stimtype_switch,group=stimtype_switch))+
...
map_switch_sub = fc_df_switch%>%
  ...
  aggregate_data(groupbyvars = c("subid","stimtype_switch"),
                 yvars = c("resp_rt"),
                 additionalvars = "iscompositional",
                 stats = "mu")%>%
...
map_switch_sum = map_switch_sub%>%
  aggregate_data(groupbyvars = c("iscompositional","stimtype_switch"),
                 yvars=c("rt"),
                 stats=c("mu","se"))

ggplot(data=map_switch_sum,aes(x=iscompositional,color=stimtype_switch,group=stimtype_switch))+
...
blockfcdata_sumallb <- within(blockfcdata_sumallb, iscenter <- relevel(iscenter, ref = "center"))
blockfcdata_suballb <- within(blockfcdata_suballb, iscenter <- relevel(iscenter, ref = "center"))
blockfcdata_sumallb <- within(blockfcdata_sumallb, iscompositional <- relevel(iscompositional, ref = "compositional"))
blockfcdata_suballb <- within(blockfcdata_suballb, iscompositional <- relevel(iscompositional, ref = "compositional"))
...
```{r}
model_rt_trial_g   = lmer(resp_rt~blocknumber_withinsess+iscenter+expt_maptype+distractor_tsrespdist + (iscenter|subid)+(distractor_tsrespdist|subid) + (1|stim_id),
              data=filter(fc_df,iscompositional=="compositional"&resp_correct==TRUE))
model_acc_trial_g   = glmer(resp_correct~blocknumber_withinsess+iscenter+expt_maptype+distractor_tsrespdist + (iscenter|subid)+(distractor_tsrespdist|subid) + (1|stim_id),
              data=filter(fc_df,iscompositional=="compositional"))
#model_dist = lmer(distractor_dist~iscompositional+blocknumber_withinsess + (1 |subid),
#              data=fc_df)
export_summs(model_rt_trial_g,model_acc_trial_g,error_format = "(\u00B1{std.error})",error_pos ="same",
             model.names=c("RT in Generalizers","ACC in Generalizers"),
             coefs = c("block" = "blocknumber_withinsess",
                       "Center>nonCenter" = "iscentercenter",
                       "Cmap>Tmap"  = "expt_maptypecross maps",
                       "distractor distance to ts resploc"="distractor_tsrespdist"
