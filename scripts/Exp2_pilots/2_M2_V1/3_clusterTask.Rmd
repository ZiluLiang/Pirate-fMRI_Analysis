...
for (pid in unique(debrief_cluster_data$subid)){
  ...
  sym_info = curr_data[,c("symID","symbol_axis","symbol_axisloc","symbol_axset")]
  sym_info = sym_info%>%
    mutate(symbol_axis = paste0(symbol_axis,symbol_axset))
  sym_info = arrange(sym_info,symbol_axis)
  ...
  (p2 = ggplot(data=dist_df,aes(x=symID_1,y=symID_2))+
      ...
      scale_x_continuous(breaks=unique(dist_df$symID_1),
                         labels=paste(sym_info$symbol_axis,sym_info$symbol_axisloc,sep="_"))+
...
(p = ggplot(data=dist_df,aes(x=symID_1,y=symID_2))+
      facet_grid2(rows=vars(group),scales="free",axes="all")+
      geom_tile(aes(fill=distance))+
      scale_x_continuous(breaks=unique(dist_df$symID_1),
                         labels=paste(sym_info$symbol_axis,sym_info$symbol_axisloc,sep="_"))+
      scale_y_continuous(breaks=unique(dist_df$symID_2),
